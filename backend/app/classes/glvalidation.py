from app.tables import GLAccounts, GLDimensions, GLDimensionValues, GLAccountDimensions, GLAccountDimensionRules, GLAccountDimensionRuleValues
...
class GLValidation:
    ...
    async def validate_account(self):
        ...
        if not account:
            raise AppException(400, f"Posting Failed: Account {self.account_id} does not exist")

        if not account.is_active:
            raise AppException(400, f"Posting Failed: Account {self.account_id} is inactive")

    async def validate_dimensions(self):

        acct_dims = await GLAccountDimensions.findByAccount(self.account_id)

        if not acct_dims:
            return

        # Build slot → (dimension_id, is_required) from account-dimension links
        slot_map: dict[int, tuple[int, bool]] = {}
        for ad in acct_dims:
            dimension = await GLDimensions.find(ad.dimension_id)
            if dimension and dimension.slot:
                slot_map[dimension.slot] = (ad.dimension_id, bool(ad.is_required))
        ...
        # Load all rules and their allowed values for this account upfront
